  ...
  window.RugbyInsights={render:function(D,view,team,analysis,filter){
    ...
    function renderRanks(){
      ...
      if(known.length){
        ...
        for(const [label,value] of extremes){const matches=known.filter(i=>rank(i)===value),st=stats(matches);html+='<section class="rank-extreme"><div class="card-label">'+label+'</div><div class="rank-value">No. '+value+' <span>· '+count(st.p)+'</span></div><table class="opponent-record" aria-label="Record against '+esc(label.toLowerCase())+'"><thead><tr><th scope="col">Played</th><th scope="col">Won</th><th scope="col">Drawn</th><th scope="col">Lost</th><th scope="col">Win%</th></tr></thead><tbody><tr><td>'+st.p+'</td><td>'+st.w+'</td><td>'+st.d+'</td><td>'+st.l+'</td><td>'+st.pct+'</td></tr></tbody></table><details class="quiet-detail"><summary>View '+(st.p===1?'match':'matches')+'</summary>'+table(matches,true)+'</details></section>';}
